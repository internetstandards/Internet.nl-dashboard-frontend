  ...
  <b-table :items="reports" :fields="fields" striped hover :busy="loading" v-model:sort-by="multiSortBy"
    ...
    <template #cell(urllist__name)="data">
      <a :href="`/shared/report/${data.item.public_report_code}`" target="_blank" rel="nofollow">
       <scan-type-icon :type="data.item.report_type" /> {{data.item.urllist__name}} {{data.item.urllist_name}} ({{data.item.report_type}})<br />
        {{$t("public-reports.table.Contains")}} {{$t("public-reports.table.domains", data.item.total_urls)}} <i-bi-box-arrow-in-up-right/>
      </a>
