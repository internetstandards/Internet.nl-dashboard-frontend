            ...
            <div class="testresults">
              <span class="select-deselect-category">
                  ...
                  <a @click="uncheck_fields(all_field_names_from_categories(category))">{{
                      $t("account.visible-metrics.uncheck")
                    }} </a>
              </span>
...
export default {
  ...
  methods: {
    reset_issue_filters: function () {
      http.get(`/api/v1/settings/account/report`).then(data => {
    ...
    save_visible_metrics() {
      ...
      * */
      ...
      http.post(`/api/v1/settings/account/report`, {'filters': this.issue_filters}).then(server_response => {
